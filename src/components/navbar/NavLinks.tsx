                ...
                <div key={idx} className='group'>
                    <div className="">
                        <button type='button' className={`${pathname === link?.url || pathname.startsWith(link.url + "/") ? "pointer-events-none text-secondary font-extrabold" : "text-white hover:text-secondary"} h-full gap-1 uppercase text-lg md:text-[13px] relative flex items-center px-3 py-6 transition-all`} onClick={() => {
                            ...
                            <p className='flex items-center' >{link.title}{link.dropdown && <BiChevronDown className='text-lg' />}</p>
                        </button>
                    </div>
                                    ...
                                    <div className="w-full bg-white rounded-b-2xl overflow-hidden shadow-md shadow-black/20 ">
                                        {link.dropdown?.map((sublink: any, idx: number) => (
                                            <Link
                                                ...
                                                className={`${pathname === sublink?.url || pathname.startsWith(sublink.url + "/") ? "pointer-events-none text-blue" : "text-blue hover:text-blue hover:bg-secondary"
                    ...
                    <div className={`${title === link.title ? 'md:hidden' : 'hidden'}`}>
                        {link.dropdown?.map((sublink: any, idx: number) => (
                            <Link
                                ...
                                className={`${pathname === sublink?.url || pathname.startsWith(sublink.url + "/") ? "pointer-events-none text-white" : "text-white"
