...
export default function Page({ params }: { params: { title: string } }) {
   ...
   function restoreUrl(formattedStr: string) {
      return formattedStr.replace(/-/g, " ").replace(/\b\w/g, (char) => char.toUpperCase());
               ...
               <div key={i} className="space-y-20 relative bg-secondary/10 pt-8 text-white">
                  <MediaQuery>
                     <div className=" space-y-6 mb-14">
                        <h1 className="text-[clamp(1rem,10vw,3rem)] leading-10 md:leading-normal font-black py-2 font-mitr">
                           {i.header}
                        </h1>
                     ...
                     <div className=" md:grid grid-cols-[2fr_1fr] lg:gap-10 items-center my-6">
                        <div
                           className={`mb-6 lg:mb-0 ${
                              i.image.length < 2 ? "grid-cols-1" : "grid grid-cols-2"
                        ...
                        >
                           {i.image?.map((pic: string) => (
                              <div
                                 key={pic}
                                 className={`${
                                    i.image.length < 2 ? "h-96" : "h-56 "
                                 } h-56 w-full relative`}
                              >
                                 <Image
                                    src={pic}
                                    alt="img"
                        ...
                        <div className="space-y-4">
                           ...
                           <ul>
                              {i.help?.list?.map((item: [], idx: number) => (
                                 <li key={idx} className="flex gap-2 items-center text-4 font-bold">
                                    <FaCircleDot className="text-dirtyOrange text-md" />
                                    {item}
                                 </li>
                              ))}
                           </ul>
                  ...
                  <div className="space-y-6 bg-white py-12">
                     <MediaQuery>
                        <h1 className="text-5xl text-blue font-black py-2 font-mitr">
                           Our Approach
                        </h1>
