...
const Page = () => {
   ...
   const tabTitle = porfolioData?.portfolio
      ?.map((items: any) => items.type)
      .flat()
   ...
   return (
      <div className="bg-themeColor pb-28">
         <ThemeBackground heading="our portfolio">
            <p>
               Explore our portfolio to see how we’ve helped clients enhance their online presence.
               From striking designs to effective strategies, our work showcases the results we can
               achieve for your business.
            </p>
         </ThemeBackground>

         <MediaQuery>
            <div className="w-full mt-6 sm:mt-0 bg-white overflow-hidden overflow-x-scroll rounded-full mb-12">
               <div className="flex items-center justify-between p-2 px-6">
                  {cat.map((title: string, idx: number) => (
                     <button
                        ...
                        className={`uppercase h-12 font-bold text-themeColor p-2 rounded-lg transition-all text-nowrap ${
                           tab === idx && "bg-label text-md font-black border-2 border-themeColor"
                        }`}
                     >
                        {title}
                     </button>
